      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-4">
          <div>
            ...
            <p className="text-white/40 max-w-md">A journey through high-performance engineering and AI integration.</p>
          </div>
        </div>
              ...
              <div className="grid md:grid-cols-[200px_1fr] gap-8">
                ...
                <div className="space-y-6">
                  {exp.roles.map((role, rIdx) => (
                    <div key={rIdx} className="p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/[0.07] transition-all group">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4">
                        <h4 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors">{role.title}</h4>
                        <span className="text-sm font-mono text-white/40">{role.dates}</span>
                      </div>
                      {role.bullets && (
                        <ul className="space-y-2 mb-4">
                          {role.bullets.map((bullet, bIdx) => (
                            <li key={bIdx} className="text-sm text-white/60 flex gap-2">
...
const Projects = () => {
  return (
    <section id="projects" className="py-24 relative overflow-hidden">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-white mb-16">Key Projects</h2>
        <div className="grid grid-cols-1 gap-8">
          {resumeData.projects.map((project, idx) => (
            ...
            >
              ...
              <p className="text-white/60 leading-relaxed max-w-4xl">{project.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
const Achievements = () => {
  return (
    <section className="py-24 bg-white/[0.02]">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-white mb-16 text-center">Impact Highlights</h2>
          ...
          {resumeData.achievements.map((ach, idx) => (
            ...
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                {ach.type === 'metric' ? <Award size={80} /> : ach.type === 'skill' ? <Cpu size={80} /> : <Briefcase size={80} />}
              </div>
              <div className="text-5xl font-bold text-blue-500 mb-4">{ach.value}</div>
...
const Skills = () => {
  return (
    <section id="skills" className="py-24">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-white mb-16">Skills Matrix</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {Object.entries(resumeData.skills).map(([category, items], idx) => (
            <motion.div
              ...
              className="space-y-6"
            >
              <h3 className="text-xs font-bold uppercase tracking-widest text-blue-500">{category}</h3>
              <div className="flex flex-wrap gap-2">
                {items.map((skill) => (
                  <span 
                    key={skill}
                    className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-white/80 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
    ...
    <section id="education" className="py-24 bg-white/[0.02]">
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-white mb-12 text-center">Education</h2>
    ...
    <footer className="py-12 border-t border-white/10">
      <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="text-center md:text-left">
          ...
          <p className="text-white/40 text-sm">Building the future of AI-powered systems.</p>
        </div>
        <div className="flex items-center gap-6">
          ...
          {resumeData.basics.links.map(link => (
            <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white transition-colors">
              {link.label === 'LinkedIn' ? <Linkedin size={24} /> : <Github size={24} />}
            </a>
          ))}
        </div>
        <p className="text-xs text-white/20">© {new Date().getFullYear()} Somdutt Vernekar. All rights reserved.</p>
      </div>
    </footer>
  );
};
  ...
  return (
    <div className="bg-black text-white selection:bg-blue-500 selection:text-white font-sans">
      ...
      <div className="hidden print:block bg-white text-black p-10 font-serif">
        ...
        <h2 className="text-2xl font-bold border-b-2 border-black mb-4">Summary</h2>
        <p className="mb-8">{resumeData.basics.summary}</p>
        
        <h2 className="text-2xl font-bold border-b-2 border-black mb-4">Experience</h2>
        ...
        <h2 className="text-2xl font-bold border-b-2 border-black mb-4">Projects</h2>
        ...
        <h2 className="text-2xl font-bold border-b-2 border-black mb-4">Education</h2>
